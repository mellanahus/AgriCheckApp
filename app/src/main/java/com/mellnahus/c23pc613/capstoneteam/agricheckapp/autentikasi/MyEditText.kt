        ...
        addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                ...
                if (inputType == InputType.TYPE_TEXT_VARIATION_PASSWORD + 1) {
                    if (s.toString().length < 8 ) error = "Password anda masih kurang dari 8 karakter"
                }

                if(inputType == InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS + 1) {
                    if (!Patterns.EMAIL_ADDRESS.matcher(s.toString()).matches()) error = "Masukan email yang valid"
                }
            }
        })
    }
